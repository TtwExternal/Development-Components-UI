//----------------------------------------------------------------------------------------------------;
var fileNm = "./ui_template/smt_ui/File/Input/smt_ui-File-Input-Basic-Test.js";
...
(function( url, $el_div ){
	 ...
	 */
	var _el_button__fileUploader = _this.$f0( $el_div, "fileUploader-button" );
	var _el_input__fileUploader = _this.$f0( $el_div, "fileUploader-input" );
	...
	var _addEvent = function()
	{
		...
		//파일 업로더 UI에 이벤트 리스너를 추가한다;
		window.b2link.fileReader.single__ArrayBuffer.addEvent( _el_input__fileUploader, _el_button__fileUploader, _changeNewInputElement );
	};
	...
	var _removeEvent = function()
	{
		...
		//파일 업로더 UI에 이벤트 리스너를 추가한다;
		window.b2link.fileReader.single__ArrayBuffer.removeEvent( _el_input__fileUploader, _el_button__fileUploader );
	};
	...
	var _changeNewInputElement = function()
	{
		/*/
		_el_input__fileUploader.remove();
		/*/
		_removeEvent();

		var newInput = window.b2link.html.createHTMLElement( _el_input__fileUploader.outerHTML );
		window.b2link.element.replaceElement( _el_input__fileUploader, newInput );
		try
		{
			_el_input__fileUploader.remove();
			_el_input__fileUploader = null;
		}
		catch( er )
		{
			window.TtwLog.error( "_changeNewInputElement Error : " + er );
		}

		_el_input__fileUploader = _this.$f0( $el_div, "fileUploader-input" );
		_addEvent();
		//*/
	};
