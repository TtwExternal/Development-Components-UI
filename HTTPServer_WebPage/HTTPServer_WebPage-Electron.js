console.log(1);global.REQUIRES={};global.REQUIRES.electron=require("electron");global.REQUIRES.path=require("path");global.REQUIRES.url=require("url");global.win;global.REQUIRES.electron.app.on("ready",createWindow);global.REQUIRES.electron.app.on("window-all-closed",function(e){if(process.platform !== "darwin") global.REQUIRES.electron.app.quit();});global.REQUIRES.electron.app.on("activate",function(e){if(win === null) createWindow();});function createWindow(){global.win=new global.REQUIRES.electron.BrowserWindow({
...
}});global.win.loadURL(global.REQUIRES.url.format({pathname:global.process.cwd()+"/root/index.html",protocol:"file:",slashes:true}));global.win.webContents.openDevTools();global.win.webContents.initialize();global.win.on("closed",function(e){global.win=null;});};
